...
**主な機能：**
- タスク（仕事依頼）の登録・編集・削除
- **状態は日付で自動管理（ステータス機能は廃止）**：未着手/進行中/完了の手動管理は撤去。予定日が過ぎた依頼はリストから自動的に隠れ、カレンダーに履歴として残る。色は日付の時制で表す（今日=オレンジ / 未来=青 / 過去・予定なし=グレー）
...
- **カレンダービュー**：リスト⇄カレンダーの切替タブ。月表示で各日に色ドット（今日/未来/過去）、日付タップでその日のタスク一覧、タスクタップでリストの該当カードへジャンプ（過去の依頼はリストに無いので編集画面で開く）。カードの日付タップでカレンダーの該当日へ（相互リンク）
...
### 6. 状態管理は日付ベース（ステータス機能は復活させない）

- ユーザー方針で **未着手/進行中/完了 のステータスUIは全廃**（集計タイル・カードのバッジ・変更ボタンを撤去）。代わりに **予定日が過ぎたらリストから自動で隠す**運用にした。ステータスのピル/フィルタを再追加しないこと。
- 判定は `isPast(task)`（過去＝リスト非表示）。**データ（`candidateDates` など）は絶対に消さない** — カレンダーに履歴として全依頼を表示するため。過ぎた候補日も配列に残す。
- リストは `listTasks`（`!isPast`）、カレンダーは `tasks`（全件＝履歴込み）を参照する。ここを取り違えると履歴が消える／過去がリストに出るので注意。
- 色分けは時制で：今日=オレンジ / 未来=青 / 過去・予定なし=グレー（`barColor`/`dateColor`）。
...
- `status` フィールドは廃止（新規作成時は付与しない）。既存データに残っていても読み飛ばすだけで害はない。
- 状態は日付から算出する（データに持たない）：`isPast(task)` が「予定日が過ぎた＝リストから隠す」判定、`barColor(task)`/`dateColor(d)` が時制の色分け。
  - 期間指定：終了日（未設定なら開始日）が今日より前なら過去。
  - 候補日：**全ての候補日が過去になったら**過去（1つでも今日以降が残ればリストに表示し続ける）。過ぎた候補日はデータからは消さない＝カレンダーに履歴として残す。
  - 日付なしは過去扱いしない（予定未定としてリストに残す）。
